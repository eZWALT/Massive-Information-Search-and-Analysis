...
def computePageRanksNIterations(Lambda: float, Epsilon: float, n_iterations = -1) -> int:
    ...
    while(not stop_condition):
        ...
        if n_iterations == -1:   
            ...
            stop_condition = iterations >= n_iterations

    global PageRank
    PageRank = P  
    return iterations

def computePageRanksMinMax(Lambda: float, Epsilon: float, n_iterations = -1)-> float:
    print("Computing ranking for all airports")
    n = len(airportList)
    #Initial distribution, each airport is as important as the rest
    P = [1.0/n for i in range(n)]
    ...
    global PageRank
    PageRank = P  
    return max(PageRank) - min(PageRank)

def computePageRanksAvg(Lambda: float, Epsilon: float, n_iterations = -1) -> float:
    print("Computing ranking for all airports")
    n = len(airportList)
    #Initial distribution, each airport is as important as the rest
    P = [1.0/n for i in range(n)]
    ...
    global PageRank
    PageRank = P  
    return sum(PageRank)/float(len(PageRank))
...
def main(argv=None):
    ...
    #epsilon_values = np.logspace(-1, -20, num=10)
    #lambda_values = np.linspace(0, 0.98, num=10)

    lambda_values = [ 0.1, 0.3, 0.45, 0.5, 0.65, 0.85, 0.9]
    epsilon_values = [10**(-2) , 10**(-4), 10**(-6), 10**(-9), 10**(-12), (10)**(-15)]
    for l in lambda_values:
        for e in epsilon_values:
            x.append(l)
            y.append(e)
            z.append(computePageRanksMinMax(l,e))
    ...
    ax.set_zlabel('Max-Min PageRank Diff')
    ax.set_title('Max-Min PageRank diff as function of Lambda and Epsilon')
    plt.show()
    """
    """
    x = []
    ...
    for l in lambda_values:
        for e in epsilon_values:
            x.append(l)
            y.append(e)
            z.append(computePageRanksNIterations(l,e))
    ...
    num_iterations = np.linspace(10, 2000, num=25, dtype=int)
    timers = []
    for iter in num_iterations:
        time1 = time.time()
        computePageRanksNIterations(Lambda,Epsilon,iter)
        time2 = time.time()
        timers.append(time2-time1)
    plt.plot(num_iterations, timers, color="red")
    ...
    #lambda experiment
    lambda_values = np.linspace(0, 0.98, num=10)
    iterations_lambda = [computePageRanksNIterations(l, 10**(-12)) for l in lambda_values]
    ...
    iterations_epsilon = [computePageRanksNIterations(0.85, e) for e in epsilon_values]
    ...
    #basic main
    time1 = time.time()
    num_iterations = computePageRanksNIterations(Lambda, Epsilon)
    time2 = time.time()
    execution_time = time2-time1
    outputPageRanks(k)
    print("#Iterations:", num_iterations)
    print("Time of computePageRanksNIterations():", execution_time)
